// components/dashboard/finance/TrendIndicator.tsx
...
const TrendIndicator: React.FC<TrendIndicatorProps> = ({ currentValue, previousValue, positiveIsGood = true }) => {
    if (previousValue === 0) {
        if (currentValue > 0) return <span className="text-xs font-semibold text-green-500 flex items-center gap-1"><TrendUpIcon className="w-3 h-3"/> Novo</span>;
